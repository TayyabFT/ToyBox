...
export function ConciergeAlertBar({ alert, onHandle }: ConciergeAlertBarProps) {
  return (
    <div className="flex flex-col gap-4 rounded-xl border border-accent/10 bg-card px-5 py-4 sm:flex-row sm:items-center">
      ...
      <button
        type="button"
        onClick={onHandle}
        className="font-roboto flex shrink-0 cursor-pointer items-center gap-2 self-start rounded-lg border border-accent/10 bg-surface px-4 py-2.5 text-[10px] font-semibold tracking-[0.12em] text-primary uppercase transition-colors hover:border-primary/40 hover:bg-accent/8 sm:self-center"
      >
        Handle Now
