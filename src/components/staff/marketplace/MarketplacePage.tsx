...
import { staffMarketplaceApi } from "@/api/staffMarketplace.api";
import { MarketplaceDeleteVehicleModal } from "@/components/admin/marketplace/MarketplaceDeleteVehicleModal";
import { MarketplaceOfferDetailModal } from "@/components/admin/marketplace/MarketplaceOfferDetailModal";
import { MarketplaceOffersPanel } from "@/components/admin/marketplace/MarketplaceOffersPanel";
import { MarketplacePageHeader } from "@/components/admin/marketplace/MarketplacePageHeader";
import { MarketplaceTabs } from "@/components/admin/marketplace/MarketplaceTabs";
import { MarketplaceVehicleFormModal } from "@/components/admin/marketplace/MarketplaceVehicleFormModal";
import { MarketplaceVehiclesPanel } from "@/components/admin/marketplace/MarketplaceVehiclesPanel";
import type {
...
export function MarketplacePage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [vehiclesResponse, offersResponse] = await Promise.all([
        staffMarketplaceApi.getVehicles(),
        staffMarketplaceApi.getOffers(),
      ]);
  ...
  async function openEditVehicle(vehicle: MarketplaceVehicleItem) {
    ...
    try {
      const response = await staffMarketplaceApi.getVehicle(vehicle.id);
  ...
  async function handleVehicleSubmit(form: Parameters<
    typeof staffMarketplaceApi.createVehicle
  >[0]) {
    setVehicleSubmitting(true);

    try {
      if (editingVehicle) {
        await staffMarketplaceApi.updateVehicle(editingVehicle.id, form);
        showSuccess("Vehicle updated");
      } else {
        await staffMarketplaceApi.createVehicle(form);
  ...
  async function handleDeleteVehicle() {
    ...
    try {
      await staffMarketplaceApi.deleteVehicle(deletingVehicle.id);
  ...
  async function openOfferDetail(
    ...
    try {
      const response = await staffMarketplaceApi.getOffer(offer.id);
  ...
  async function handleOfferAction(
    ...
    try {
      ...
      await staffMarketplaceApi.actOnOffer(offer.id, body);
