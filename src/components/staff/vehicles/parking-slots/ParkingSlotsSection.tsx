...
export function ParkingSlotsSection({
  ...
  const loadSlots = useCallback(async () => {
    ...
    try {
      const [listResponse, summaryResponse] = await Promise.all([
        staffParkingApi.getSlots(),
        showSummary ? staffParkingApi.getSummary() : Promise.resolve(null),
      ]);
  ...
  const loadDetail = useCallback(async (id: string) => {
    ...
    try {
      const response = await staffParkingApi.getSlot(id);
  ...
  const handleDeleteSlot = useCallback(async () => {
    ...
    try {
      await staffParkingApi.deleteSlot(selectedId);
