    ...
    <div className="space-y-2">
      ...
      {canAddMore && (
        <button
          type="button"
          onClick={openPicker}
          className={`flex w-full cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed bg-elevated px-4 py-8 text-center transition-colors hover:border-primary/35 ${
      ...
      {files.length > 0 && (
        <div className="grid grid-cols-3 gap-2.5 pt-1">
          {files.map((file, index) => (
            <div
              key={`${file.name}-${index}`}
              className="relative aspect-square overflow-hidden rounded-lg border border-accent/15 bg-elevated"
            >
              {previews[index] && (
                <img
                  src={previews[index]}
