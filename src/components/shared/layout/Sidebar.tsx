...
const navIcons: Record<string, (active: boolean) => ReactNode> = {
  ...
  bookings: (active) => (
    <VehicleCalendar color={active ? "var(--accent)" : "var(--muted)"} />
  ),
...
export function Sidebar({ role }: { role: UserRole }) {
  ...
  return (
    <aside className="sidebar fixed flex h-screen w-[330px] shrink-0 flex-col overflow-hidden border-r border-accent/8 bg-background">
